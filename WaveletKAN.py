...
class WaveletKANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if x.shape[-1] != self.in_features:
            raise ValueError(f"Expected input's last dim to be {self.in_features}, got {x.shape[-1]}")
        x_flat = x.reshape(-1, self.in_features)

        # 计算基础线性变换的输出
        base_output = F.linear(self.base_activation(x_flat), self.base_weight)

        # 计算小波变换的输出
        wavelet_output = self.wavelet_transform(x_flat)  # (batch_size, out_features)
